...
class Matemat:
    ...
    def waitForReply(self,reply):
        ...
        self.log.log('waitForReply(): reply=%s' % reply)
        while True:
            msg = self.interface.readMessage()
            if msg == False:
                return False
            if msg in reply:
                self.log.log('waitForReply(): msg=%s' % msg)
                return msg


    def writeLCD(self, msg):
        self.log.debug('writeLCD(): invoked')
        self.log.log('writeLCD(): msg=%s' % msg)
        msg = "d"+msg
    ...
    def getPriceline(self):
        ...
        while True:
            ...
            if msg == False:
                return -1
            if msg[0] == 'p':
                self.log.log('getPriceline(): priceline=%s' % msg[2])
                return int(msg[2])

    def serve(self,priceline):
        self.log.debug('serve(): invoked')
        self.log.log('serve(): priceline=%s' % priceline)
